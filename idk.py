...
DIR_SETUP_S = 0.002
EDGE_DWELL_S = 0.2
TEST_LOOPS = 1  # 1 = run once, 0 = run forever
...
# If Y moves opposite of expected, change 1 to 0
...
class StepperAxis:
    ...
    def _pulse_with_wave(self, steps: int, freq_hz: int, pulse_us: int) -> None:
        ...
        pulse_high = (
            pigpio.pulse(step_mask, 0, high_us)
            if high_phys else
            pigpio.pulse(0, step_mask, high_us)
        )
        pulse_low = (
            pigpio.pulse(step_mask, 0, low_us)
            if low_phys else
...
def run_y_path(y_axis: StepperAxis) -> None:
    loop_count = 0

    while TEST_LOOPS == 0 or loop_count < TEST_LOOPS:
        loop_count += 1
        print(f"\nY test loop {loop_count}")

        for label, distance_cm in Y_PATH_CM:
            est_steps = int(round(abs(distance_cm) * y_axis.cfg.steps_per_cm))
            print(f"{label}: {est_steps} steps @ {STEP_FREQ_HZ} Hz")
            y_axis.move_cm(distance_cm, STEP_FREQ_HZ, STEP_PULSE_US)
